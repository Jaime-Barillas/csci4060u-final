...
int64_t bin_index(int64_t x, int64_t y, const SimCtx *ctx) {
    // Account for particles on the right/bottom edge. These would result in a
    // buffer overflow otherwise.
    // x = SDL_clamp(x, 0, ctx->window_width - 1);
    // y = SDL_clamp(y, 0, ctx->window_height - 1);
...
void sort_by_cell(SimCtx *ctx) {
    ...
    for (int64_t i = 1; i < ctx->pcount; i++) {
